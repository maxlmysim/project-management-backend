...
export const createTask = async (req: Request, res: Response) => {
  ...
  const bodyError = checkBody(req.body, ['title', 'order', 'description', 'userId', 'users', 'isDone'])
  if (bodyError) {
...
export const updateTask = async (req: Request, res: Response) => {
  const guid = req.header('Guid') || 'undefined';
  const initUser = req.header('initUser') || 'undefined';
  const bodyError = checkBody(req.body, ['title', 'order', 'description', 'userId', 'columnId', 'users', 'isDone'])
  if (bodyError) {
    return res.status(400).send(createError(400, "bad request: " + bodyError));
  }
  ...
  try {
    const updatedTask = await taskService.updateTask(req.params.taskId, {
      title,
      order,
      description,
      userId,
